...
<html>
	...
	<body>
		<nav class="mui-bar mui-bar-tab">
			<a id="defaultTab" class="mui-tab-item <?php if(!$pod){?>mui-active<?php }?>" href="#pending_payment">
				<span class="mui-tab-label">待付款</span>
			</a>
			<a class="mui-tab-item <?php if($pod==1){echo 'mui-active';}?>" href="#pending_receiving">
				<span class="mui-tab-label">待收货</span>
			</a>
			<a class="mui-tab-item <?php if($pod==2){echo 'mui-active';}?>" href="#already_receipt">
				<span class="mui-tab-label">已收货</span>
			</a>
		</nav>
		<!--第一个子页面开始(待收货)-->
		<div id="pending_payment" class="mui-control-content <?php if(!$pod){?>mui-active<?php }?>">
		...
		<!--第一个子页面结束-->
		<!--第二个子页面开始-->
		<div id="pending_receiving" class="mui-control-content <?php if($pod==1){echo 'mui-active';}?>">
			<div class="list-container">
		...
		<!--第二个子页面结束-->
		<!--第三个子页面开始-->
		<div id="already_receipt" class="mui-control-content <?php if($pod==2){echo 'mui-active';}?>">
		<div class="list-container">
						...
						function(od){
							if(od.code==1){
									layer.msg("取消成功",{time:500},
										function(){
											window.location.href="ordertab.php";
		...
		//确认成功
			function finishOrder(odnum){
				$.post(
						...
						function(od){
							if(od.code==1){
									layer.msg("确认成功",{time:500},
										function(){
											window.location.href="ordertab.php?pod=2";
