...
class RKMongo
{
	...
	//获取id集合内的用户位置坐标
	function getGPSAddressAll($userType,$myGPS,$userId)
	{
		$arr=array();
		foreach ($userId as $v){
			$arr[]=(int) $v;
...
// 		try{
// 				$collection = $this->mongo->{$database}->selectCollection($tableName);
// 				$cursor=$collection->find(array('_id'=>array('$in'=>$arr)));
// 				while( $cursor->hasNext() ) {
// 					$value=$cursor->getNext();
// 					$result[$value['_id']]=$value['gps'];
// 				}
// 				return  $result;
...
// 			}
			...
			$queryCondition = array('_id'=>array('$in'=>$arr));
