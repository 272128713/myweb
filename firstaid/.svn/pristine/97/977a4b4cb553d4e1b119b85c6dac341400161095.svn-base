	...
	*/
	public function getUserInfoByUid($uidlist) {
	    ...
	    $condition = " a.user_id in (" . $uidlist . ")";
 		$sql = "SELECT a.user_id,a.client_type,a.mid,a.push_service_type,a.mec_ip,a.mec_port,a.lps_ip,a.lps_port,b.mobile FROM 
  		sky_first_aid.user_session_info as a, sky_user_data_master.user_base_info as b where a.user_id=b.user_id and $condition";

// 		 $sql1="select token from temp_tokens";
// 		 $tokens=$this->conn->getAll($sql1);
//          foreach ($tokens as $key=>$value){ //把救命稻草的id拼接成字符串
...
// 	     $sql = " SELECT a.user_id,a.client_type,a.mid,a.push_service_type,a.mec_ip,a.mec_port,a.lps_ip,a.lps_port,b.mobile
//          FROM sky_first_aid.user_session_info as a, sky_user_data_master.user_base_info as b
//          where a.user_id=b.user_id and $condition  and
//          (a.push_service_type=2 or (a.push_service_type=1 and a.mid in ($token)))";
...
// 	public function addTempTokens($mid){
		
// 		$sql="insert into temp_tokens(token) values('$mid')";
// 		$this->conn->execute($sql);
		
// 	}

	public function getUserPav($uid){
		$sql = "SELECT image_ver pav FROM sky_user_data_master.user_version_info WHERE user_id=$uid";
