...
<body>
	...
	<?php 
	    ...
	    foreach($configs as $key=>$config){
	        ...
	        //$strHtml .= "<form action='" . $configs["global"]["host"] . $keyTrue."?XDEBUG_SESSION_START=ECLIPSE_DBGP&KEY=14399828018291". "' method='post' target='$key' " . $formEncoding . ">";
	      ...
	      $strHtml .= "<form action='" . $configs["global"]["host"] . $keyTrue. "' method='post' target='$key' " . $formEncoding . ">";
	        ...
	        $strHtml .= "<div style='position:absolute;bottom:0px;width:90%'><fieldset style='border:1px solid #369;bottom:0px'><legend>接口返回值</legend><iframe name='$key' frameborder='0' style='width:99%;height:100px'></iframe></fieldset></div>";
	        $strHtml .= "</fieldset>";
	    }
	    echo $strHtml;
	?>
