...
class IndexController extends CommonController
{
    ...
    public function  detail()
    {
        ...
        //评价最热
        $hotcom = M()->query("select a.*,b.user_name,c.thumbnail_image_url from article_evaluation as a  LEFT JOIN user_base_info as b ON b.user_id=a.user_id LEFT JOIN user_version_info as c ON c.user_id=a.user_id WHERE
a.article_id=$aid and a.up_num >0 ORDER BY a.up_num desc limit 5 ");
        ...
        //评价最新
        $newcom = M()->query("select a.*,b.user_name,c.thumbnail_image_url from article_evaluation as a  LEFT JOIN user_base_info as b ON b.user_id=a.user_id LEFT JOIN user_version_info as c ON c.user_id=a.user_id WHERE
a.article_id=$aid and a.id not in($condition) ORDER BY a.id desc limit 5 ");
    ...
    public function  evalute()
    {
        ...
        if (!isset($_POST['content'])) {
            if ($_GET['type'] == 1) {
                $com = M()->query("select a.*,b.user_name,c.thumbnail_image_url from article_evaluation as a  LEFT JOIN user_base_info as b ON b.user_id=a.user_id LEFT JOIN user_version_info as c ON c.user_id=a.user_id WHERE
...
a.article_id=$aid ORDER BY a.up_num desc limit $limit ," . C('PAGE_NUM'));
            ...
            foreach($com as $key=>$v){
                if($v['thumbnail_image_url']==''){
                    $com[$key]['thumbnail_image_url']=__ROOT__.'/Public/Article/images/MOO/data/default.jpg';
                }else{
                    $com[$key]['thumbnail_image_url']='http://'.C('IMG_HOST').str_replace('M00','MOO/data',$v['thumbnail_image_url']);
                }
            }
            $this->com = $com;
            //文章相关
