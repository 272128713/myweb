...
$handler = set_error_handler("sysErrorHandler",E_ALL&~E_DEPRECATED);
 function create_time_id(){
	list($usec, $sec) = explode(' ', microtime());
	$random_seed= (float) $sec + ((float) $usec * 100000);
	srand($random_seed);
	$randval = rand(100000,999999);
	return date('YmdHis').$randval;
}
########################################################
##
##		以下函数属于thinkphp框架调整后的公共函数
##
########################################################
/**
 * 生成一个全局唯一的19位纯数字的订单
 ...
 * 返回合法的页面
 */
function return_right_page($page){
	$page=intval($page);
	if(!is_int($page)){
		return 1;
	}else{
...
//remove non-BMP characters 表情过滤函数
