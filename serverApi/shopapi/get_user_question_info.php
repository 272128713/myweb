...
$userId = $dbMaster->getOne($sql);
// $username = $sessionArr['rn'];
$sql = "select user_name from user_base_info where user_id = '$userId'";
$username = $dbSso->getOne($sql);
...
foreach ($result as $k=>$v){
 ...
 if($result_arr){
 	if($read_uid==$userId){
	 	$sql ="update user_question_info set isRead = 1 where id = '$uqid'";
	 	$dbMaster->execute($sql);
 	}
 	$sql = "select id from doctor_answer_info where doctor_id = '$read_uid' and question_id = '$uqid'";
 	$result_readid = $dbMaster->getCol($sql);
 	foreach ($result_readid as $vid){
 		$sql = "update user_question_for_answer_info set isRead = 1 where answer_id = '$vid'";
 		$dbMaster->execute($sql);
 	}
 }
