...
foreach($orderid as $k=>$v){
    ...
    $totalprice = $totalprice + $pricedata;
    $sql = "insert into user_buy_goods_info
          (user_id,goods_id,price,buy_time,numbers,order_nums,pay_state,logistics_state,order_state,send_address_id,pay_user_id,pay_type)
          VALUES
          ('$suid','$v','$pricedata',NOW(),1,'$ordernum','0','0','0','0','$userId','-1')";

    $result = $dbMaster->execute($sql);
}

ErrCode::echoJson('1','success',array('odnum'=>$ordernum,'totalprice'=>$totalprice));
