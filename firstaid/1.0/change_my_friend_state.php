...
//功能：标志位为黑名单(flag = 0)时删除监控 2016/01/28 Alan
if (0 == $params['flag']) {
   $r =  deleteSupervise($uid,$params['uid']);

    if (!$r) logger()->error(sprintf("用户%s主动监控删除失败", $uid));
...
function deleteSupervise($uid,$fid)
{
    $sql = <<<T_ECHO
DELETE FROM
    user_supervise_info
WHERE
        (prey_id = $uid AND hunter_id = $fid)
    OR
        (prey_id = $fid AND hunter_id = $uid)
T_ECHO;
