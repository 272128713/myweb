...
function delFriendList($userID, $friendList) {
    ...
    DELETE FROM user_friend_list
        WHERE user_id=$userID AND friend_id IN ($friendList) AND is_friend = 1
T_ECHO;
    $rightSql =<<<T_ECHO
    DELETE FROM user_friend_list
        WHERE user_id in ($friendList) AND friend_id=$userID  AND is_friend = 1
T_ECHO;
    try {
        //删除相关的失联防范和电子围栏
...
function delBlackList($userID, $friendList) {
    ...
    $leftSql = <<<T_ECHO
    DELETE FROM user_friend_list
        WHERE user_id = $userID AND friend_id IN ($friendList)  AND is_friend = 0
T_ECHO;
    $rightSql =<<<T_ECHO
    DELETE FROM user_friend_list
        WHERE user_id in ($friendList) AND friend_id = $userID  AND is_friend = 0
T_ECHO;
    try {
