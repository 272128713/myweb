...
//T_ECHO;
    $sql = <<<T_ECHO
SELECT s.id FROM `sky_first_aid`.`user_supervise_info` AS s WHERE s.prey_id="{$uid}" AND s.hunter_id="{$loginuid}" AND s.TYPE=1 AND s.state=1
T_ECHO;
            ...
            foreach ($BeMonFriends as $bmFriend) {
                for ($i = 0; $i < count($friends); $i++) {
                    if ($friends[$i]['friend_id'] == $bmFriend['id']) {
                        ...
                        $pery_id = $bmFriend['id'];
                        $sql = "DELETE FROM `sky_first_aid`.`user_supervise_info` WHERE hunter_id=$uid AND prey_id=$pery_id AND state=0";
                        $db->execute($sql);
...
T_ECHO;
    ...
    if ($res_all = $db->getAll($sql)) {
            /** 查询出所有监控我的人 */
            $sql = <<<T_ECHO
SELECT hunter_id as friend_id FROM `sky_first_aid`.`user_supervise_info` WHERE prey_id={$uid} AND state=1
T_ECHO;
...
T_ECHO;
         ...
         */
        $my_hunter_all = "SELECT COUNT(*) as a FROM `sky_first_aid`.`user_supervise_info` WHERE prey_id=$uid AND state=1";
        ...
        $hunter_num_all = $db->getRow($my_hunter_all);
        ...
        $retarr['msg']='请求成功';
        $retarr['fnum']=$num;
        $retarr['hunter_num_all']=$hunter_num_all['a'];
...
function getMyGuardInfo($uid)
{
    $dbObj = new DatabaseManager();
    $db = $dbObj->getConn();
    $sql_m = "SELECT a.hunter_id user_id , b.user_name rn ,b.mobile mobile,c.base_ver piv ,c.image_ver pav
            FROM `sky_first_aid`.`user_supervise_info` as a,
                 `sky_user_data_master`.`user_base_info` as b,
                 `sky_user_data_master`.`user_version_info` as c
            WHERE a.prey_id=$uid AND a.type = 1 AND a.state=1 AND a.hunter_id = b.user_id AND b.user_id = c.user_id";
    $sql_s = "SELECT a.hunter_id user_id , b.user_name rn ,b.mobile mobile,c.base_ver piv ,c.image_ver pav
            FROM `sky_first_aid`.`user_supervise_info` as a,
                 `sky_user_data_master`.`user_base_info` as b,
                 `sky_user_data_master`.`user_version_info` as c
            WHERE a.prey_id=$uid AND a.type = 0 AND a.state=1 AND a.hunter_id = b.user_id AND b.user_id = c.user_id";
    $res_m = $db->getAll($sql_m);
    $res_s = $db->getAll($sql_s);

    $rkMongo = new RKMongo();
    $rkMongo->connect();

    for ($i = 0; $i < count($res_m); $i++){
        $temp = $rkMongo->getGPSAddress(1, $res_m[$i]['user_id']);
        $res_m[$i]['gps'] = empty($temp['gps']) ? array() : $temp['gps'];
    }
    for ($i = 0; $i < count($res_s); $i++){
        $temp = $rkMongo->getGPSAddress(1, $res_s[$i]['user_id']);
        $res_s[$i]['gps'] = empty($temp['gps']) ? array() : $temp['gps'];
    }
