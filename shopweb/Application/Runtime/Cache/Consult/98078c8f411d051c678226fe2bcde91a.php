...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/sanlingyi/Public/Consult/css/searchresult.css"/>
	...
	<body>
		...
		<header class="mui-bar mui-bar-nav" style="background:#30c6df;position: fixed;">
			<a class="mui-icon mui-icon-left-nav mui-pull-left" onclick="logoutConsultings()"></a>
			<h1 class="mui-title" style="color:#fff" onclick="logoutConsultings()">在线咨询</h1>
            <div class="right">
                ...
                <a style="float:left;color: #fff;display: block;height: 44px;width:3em;text-align:center;line-height:44px;font-size:16px;" href="<?php echo U('SelfConsult/index');?>" >记录</a>
        ...
        -->
	        <div class="mui-pull-left mui-segmented-control cc">
				<a href="<?php echo U('index',array('svalue'=>$search));?>" class="mui-control-item <?php if(!isset($_GET['type'])) echo 'mui-active'; ?>">最新</a>
				<a href="<?php echo U('index',array('order'=>'answer_num','type'=>1,'svalue'=>$search));?>" class="mui-control-item <?php if($_GET['type']==1) echo 'mui-active'; ?> ">最热</a>
				<a href="<?php echo U('index',array('type'=>2,'svalue'=>$search));?>" class="mui-control-item <?php if($_GET['type']==2) echo 'mui-active'; ?> ">已解决</a>
			    <?php $type=$_GET['type']; ?>
            </div>
				<a href="<?php echo U('Search/index');?>"><img class="mui-pull-right search" src="/sanlingyi/Public/Consult/images/sousuo.png"/></a>
