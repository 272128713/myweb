...
class MyChatController extends CommonController {
    ...
    */
    public  function  index(){
        ...
        //我的评价
        $mc=M()->query("select a.*,c.thumbnail_image_url,d.title,d.article_from from article_evaluation as a  LEFT JOIN user_version_info as c ON c.user_id=a.user_id LEFT JOIN com_top_article as d on d.id=a.article_id
 where a.user_id=$uid ORDER BY a.id desc limit ".C('PAGE_NUM'));
        $this->mc=$mc;
        //我的信息
        $my=M('user_base_info')->field('user_name,live_place')->where(array('user_id'=>$uid))->find();
        $this->me=$my;
        $db=M('com_sic_region_info');
        $this->province=$db->find(substr($my['live_place'],0,2).'0000');

        $this->city=$db->find(substr($my['live_place'],0,4).'00');
        $pic=M('user_version_info')->where(array('user_id'=>$uid))->getField('thumbnail_image_url');
        if($pic==''){
