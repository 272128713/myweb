...
class MecManagerModel extends TcpConnectionModel{
    private $logger;
    private $sender;
    ...
    private function initAccepter($accepters){
        foreach ($accepters as $accepter){
            if(!isset($this->TcpPool[$accepter["mec_ip"] . $accepter["mec_port"]])){
	            $tcpConnection = new \Consult\Model\TcpConnectionModel($accepter["mec_ip"], $accepter["mec_port"]);
            	//$tcpConnection = D("TcpConnection:".$accepter["mec_ip"].",". $accepter["mec_port"]);
...
/*        $ttl = $this->config->getConfig("ttl_" . $this->msgObject["type"]);
	...
	$ttl = 1296000;
        foreach ($this->TcpPool as $key => $tcpPool){
            ...
            \Think\Log::record('消息'.json_encode($message));
